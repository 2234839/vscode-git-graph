...
export class GitGraphView extends Disposable {
    ...
    switch (msg.command) {
      ...
      case 'commitDetails':
        let data = await Promise.all<GitCommitDetailsData, string | null>([
          msg.commitHash === UNCOMMITTED ? this.dataSource.getUncommittedDetails(msg.repo)
          : msg.stash === null ?
  ...
  private getHtmlForWebview() {
    ...
    if (this.dataSource.isGitExecutableUnknown()) {
      ...
      const stickyClass = initialState.config.stickyHeader ? 'sticky' : '';
      body = `<body>
			<div id="app" data-sticky="${stickyClass}"></div>
