...
export class RepoManager extends Disposable {
  ...
  constructor(
  	...
  	onDidChangeConfiguration: GgEvent<vscode.ConfigurationChangeEvent>,
  	logger: Logger
  ) {
  	super();
  ...
  private async startupTasks() {
  	...
  	await Promise.all(
  		Object.keys(this.repos).map((repo) => {
  ...
  public registerRepo(path: string, loadRepo: boolean) {
  	return new Promise<{ root: string | null; error: string | null }>(async (resolve) => {
  		...
  		if (root === null) {
  			resolve({ root: null, error: t('rmFolderNotGitRepo', path) });
  		} else if (typeof this.repos[root] !== 'undefined') {
  			resolve({
  				root: null,
  				error: t('rmFolderAlreadyIncluded', path, root)
  			});
  		} else {
        ...
        file.exportedAt > state.lastImportAt
            ...
            isRepoNew ? t('rmYes') : (
            	await vscode.window.showInformationMessage(
            		t('rmNewerConfigPrompt', state.name || getRepoName(repo)),
            		t('rmYes'),
            		t('rmNo')
            	)
            );
  				if (this.isKnownRepo(repo) && action) {
  					...
  					this.extensionState.saveRepos(this.repos);
  					if (!isRepoNew && action === t('rmYes')) {
  						showInformationMessage(
  							t('rmConfigRepoSuccessVar', state.name || getRepoName(repo))
  						);
  					}
  					return true;
  				}
		...
		fs.mkdir(vscodePath, (err) => {
			if (!err || err.code === 'EEXIST') {
				...
				fs.writeFile(configPath, JSON.stringify(file, null, 4), (err) => {
					if (err) {
						reject(t('rmUnableWriteConfigVar', getPathFromStr(configPath)));
					} else {
						resolve(t('rmExportSuccessVar', getPathFromStr(configPath)));
					}
				});
			} else {
				reject(
					t('rmCheckDirErrorVar', getPathFromStr(vscodePath))
				);
			}
		});
	});
...
function applyExternalConfigFile(file: Readonly<ExternalRepoConfig.File>, state: GitRepoState) {
	...
	if (typeof file.includeCommitsMentionedByReflogs !== 'undefined') {
      ...
      file.includeCommitsMentionedByReflogs ? BooleanOverride.Enabled : BooleanOverride.Disabled;
	}
	...
	if (typeof file.onlyFollowFirstParent !== 'undefined') {
		state.onlyFollowFirstParent =
      file.onlyFollowFirstParent ? BooleanOverride.Enabled : BooleanOverride.Disabled;
	}
	if (typeof file.onRepoLoadShowCheckedOutBranch !== 'undefined') {
		state.onRepoLoadShowCheckedOutBranch =
      file.onRepoLoadShowCheckedOutBranch ? BooleanOverride.Enabled : BooleanOverride.Disabled;
	}
	...
	if (typeof file.showRemoteBranches !== 'undefined') {
		state.showRemoteBranchesV2 =
      file.showRemoteBranches ? BooleanOverride.Enabled : BooleanOverride.Disabled;
	}
