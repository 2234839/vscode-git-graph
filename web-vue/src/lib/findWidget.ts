...
import { SVG_ICONS, ATTR_ERROR, CLASS_ACTIVE, CLASS_DISABLED, CLASS_TRANSITION } from './constants';
import { abbrevCommit, alterClass, findCommitElemWithId, formatShortDate, getBranchLabels, getChildNodesWithTextContent, getChildrenWithClassName, getCommitElems, handledEvent, modifyColourOpacity } from './utils';
import { UNCOMMITTED, getWindowGlobals, type GitGraphView } from '../types';
import { updateWorkspaceViewState } from '../composables/bridge';
import { t } from './i18n';

const workspaceState = getWindowGlobals().workspaceState;
  	...
  	if (this.text !== '') {
  		...
  		if (findPattern !== null && findGlobalPattern !== null) {
            ...
            ((colVisibility.author && findPattern.test(commit.author)) ||
              ...
              (colVisibility.date && findPattern.test(formatShortDate(commit.date, initialState.config.dateFormat).formatted)) ||
