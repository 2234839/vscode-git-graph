...
import { SVG_NAMESPACE, ELLIPSIS } from './constants';
import { abbrevCommit, escapeHtml, findCommitElemWithId, getBranchLabels, getCommitElems } from './utils';
import { UNCOMMITTED, type ExpandedCommit } from '../types';

/** 可选回调：关闭对话框和右键菜单（由上层注入，避免循环依赖） */
let _closeDialogAndContextMenu: (() => void) | null = null;

/** 设置 closeDialogAndContextMenu 回调 */
export function setCloseDialogAndContextMenu(fn: (() => void) | null) {
  _closeDialogAndContextMenu = fn;
}
...
export class Graph {
	...
	/** 是否合并显示本地和远程分支标签 */
	private readonly combineLocalAndRemote: boolean;
	...
	constructor(id: string, viewElem: HTMLElement, config: GG.GraphConfig, muteConfig: GG.MuteCommitsConfig, combineLocalAndRemote: boolean) {
		this.viewElem = viewElem;
		this.config = config;
		this.muteConfig = muteConfig;
		this.combineLocalAndRemote = combineLocalAndRemote;
		...
		if (id < this.commits.length && this.commits[id].hash !== UNCOMMITTED) { // Only show tooltip for commits (not the uncommitted changes)
			this.tooltipTimeout = setTimeout(() => {
				...
				if (vertexScreenY >= 5 && vertexScreenY <= this.viewElem.clientHeight - 5) {
					...
					if (_closeDialogAndContextMenu) _closeDialogAndContextMenu();
	...
	private showTooltip(id: number, vertexScreenY: number) {
		...
		if (heads.length > 0 || remotes.length > 0) {
			let branchLabels = getBranchLabels(heads, remotes, this.combineLocalAndRemote), htmlRefs: string[] = [];
