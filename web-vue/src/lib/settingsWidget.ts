...
import { addListenerToClass, alterClass, arraysStrictlyEqualIgnoringOrder, escapeHtml, formatCommaSeparatedList, getRepoName, getShowStashes, getShowTags, getIncludeCommitsMentionedByReflogs, getOnlyFollowFirstParent, getOnRepoLoadShowCheckedOutBranch, getOnRepoLoadShowSpecificBranches, observeElemScroll } from './utils';
...
export class SettingsWidget {
   ...
   */
  constructor(view: GitGraphView, dialog: Dialog, runAction: (msg: GG.RequestMessage, action: string) => void) {
  	this.view = view;
  	this.dialog = dialog;
  ...
  private render() {
  	if (this.currentRepo !== null && this.repo !== null) {
        ...
        this.repo.onRepoLoadShowSpecificBranches !== null;
  		...
  		if (getOnRepoLoadShowCheckedOutBranch(this.repo.onRepoLoadShowCheckedOutBranch, initialState.config.onRepoLoad.showCheckedOutBranch)) {
  			initialBranches.push(t('checkedOutShort'));
  		}
  		const branchOptions = this.view.getBranchOptions();
  		getOnRepoLoadShowSpecificBranches(this.repo.onRepoLoadShowSpecificBranches, initialState.config.onRepoLoad.showSpecificBranches).forEach(
  			(branch) => {
      ...
      showStashesElem.checked = getShowStashes(this.repo.showStashes, initialState.config.showStashes);
      ...
      document.getElementById('exportRepositoryConfig')!.addEventListener('click', () => {
      	this.dialog.showConfirmation(
      		...
      		() => {
      			if (this.currentRepo === null) return;
      			this.runAction({ command: 'exportRepoConfig', repo: this.currentRepo }, t('exportingRepoConfig'));
      		},
      		null
      	);
      });
