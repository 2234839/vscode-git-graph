...
import { SVG_ICONS, CLASS_ACTIVE, CLASS_LOADING, CLASS_TRANSITION, ELLIPSIS } from './constants';
import {
  addListenerToClass,
  alterClass,
...
import { sendMessage, updateGlobalViewState } from '../composables/bridge';
import { getWindowGlobals, type GitGraphView, TargetType } from '../types';
import { t } from './i18n';
import { Dialog, DialogInputType, type DialogInput, type DialogInputValue } from './dialog';
...
export class SettingsWidget {
  ...
  /** 依赖注入：runAction 回调 */
  private readonly runAction: (msg: GG.RequestMessage, action: string) => void;
   ...
   * @param runAction 执行动作的回调
   */
  constructor(
    view: GitGraphView,
    dialog: Dialog,
    runAction: (msg: GG.RequestMessage, action: string) => void,
  ) {
    this.view = view;
    this.dialog = dialog;
  ...
  private render() {
    if (this.currentRepo !== null && this.repo !== null) {
      ...
      let html =
        ...
        t('settingsInitialBranchesColon') +
        '</td><td class="leftWithEllipsis" title="' +
        initialBranchesStr +
        ' (' +
        ...
        (initialBranchesLocallyConfigured ?
          ...
          t('settingsClearInitialBranches') +
          ELLIPSIS +
          '">' +
          SVG_ICONS.close +
          '</div>'
        ...
        t('settingsIncludeReflog') +
        '</label><span class="settingsWidgetInfo" title="' +
        t('settingsIncludeReflogInfo') +
        '">' +
        SVG_ICONS.info +
        '</span><br/>' +
        ...
        t('settingsOnlyFirstParent') +
        '</label><span class="settingsWidgetInfo" title="' +
        t('settingsOnlyFirstParentInfo') +
        '">' +
        SVG_ICONS.info +
        '</span>' +
        '</div>';
      ...
      if (this.config !== null) {
        ...
        if (userNameSet || userEmailSet) {
          ...
          html +=
            ...
            '</td></tr>' +
            '<tr><td class="left">' +
            t('settingsUserEmailColon') +
            '</td><td class="leftWithEllipsis" title="' +
            escapedUserEmail +
            (userEmailSet ?
              ' (' + (userEmail.local !== null ? t('local') : t('global')) + ')'
            : '') +
            '">' +
            escapedUserEmail +
            '</td></tr>' +
        ...
        html +=
          ...
          t('settingsRemotes') +
          '</h3><table><tr><th>' +
          t('settingsRemoteHeader') +
          '</th><th>URL</th><th>' +
          t('settingsTypeHeader') +
          '</th><th>' +
          t('settingsActionsHeader') +
          '</th></tr>';
        if (this.config.remotes.length > 0) {
          ...
          this.config.remotes.forEach((remote, i) => {
            ...
            html +=
              ...
              i +
              '" title="' +
              t('clickToShowHide', hidden ? t('show') : t('hide')) +
              '">' +
      ...
      html +=
        '<div class="settingsSection"><h3>' +
        t('settingsGitGraphConfig') +
        '</h3>' +
        '<div class="settingsRow"><span class="settingLabel">' +
        t('settingsLanguage') +
        '</span>' +
        '<div class="languageSelector">' +
        '<button data-lang="auto" class="langBtn">' +
        t('settingsLanguageAuto') +
        '</button>' +
        '<button data-lang="zh" class="langBtn">' +
        t('settingsLanguageZh') +
        '</button>' +
        '<button data-lang="en" class="langBtn">' +
        ...
        SVG_ICONS.gear +
        t('settingsOpenExtSettings') +
        '</div><br/>' +
        '<div id="exportRepositoryConfig">' +
        SVG_ICONS.package +
        t('settingsExportRepoConfig') +
        '</div>' +
        '</div></div>';
        ...
        document.getElementById('deleteRepoName')!.addEventListener('click', () => {
          ...
          this.dialog.showConfirmation(
            t(
              ...
              '<b><i>' + escapeHtml(this.repo.name) + '</i></b>',
              '<b><i>' + escapeHtml(getRepoName(this.currentRepo)) + '</i></b>',
            ),
            t('yesDelete2'),
            () => {
              if (this.currentRepo === null) return;
      ...
      document.getElementById('editInitialBranches')!.addEventListener('click', () => {
        ...
        this.dialog.showForm(
          '<b>' +
            t('configureInitialBranches') +
            '</b><p style="margin:6px 0;">' +
            t('configureInitialBranchesDesc') +
            '</p><p style="font-size:12px; margin:6px 0 0 0;">' +
            t('configureInitialBranchesNote') +
            '</p>',
          [
            {
              type: DialogInputType.Checkbox,
              name: t('checkedOutBranches'),
      ...
      showStashesElem.checked = getShowStashes(
        this.repo.showStashes,
      ...
      showStashesElem.addEventListener('change', () => {
        ...
        this.view.refresh(true, false);
      });

      const showTagsElem = <HTMLInputElement>document.getElementById('settingsShowTagsCheckbox');
      showTagsElem.checked = getShowTags(this.repo.showTags, initialState.config.showTags);
      ...
      if (this.config !== null) {
        document.getElementById('editPullRequestIntegration')!.addEventListener('click', () => {
          ...
          if (this.config.remotes.length === 0) {
            this.dialog.showError(t('unableConfigPR'), t('mustHaveRemote'), null, null);
            return;
          }
  ...
  private showIssueLinkingDialog(
    ...
    let html =
      '<b>' +
      (isEdit ? t('settingsEditIssueLinking') : t('settingsAddIssueLinkingToRepo')) +
      '</b>';
    html += '<p style="font-size:12px; margin:6px 0;">' + t('issueLinkingExample') + '</p>';
    html +=
    ...
    if (!isEdit && defaultIssueRegex === null && defaultIssueUrl === null) {
      ...
      if (defaultIssueRegex !== null) {
        html += '<p style="font-size:12px">' + t('issuePrefilledDetected') + '</p>';
      }
    }
    ...
    this.dialog.showForm(
      ...
      (values) => {
        ...
        if (regExpParseError !== null) {
          this.dialog.showError(t('invalidIssueRegexp'), regExpParseError, t('back'), () => {
