...
import { SVG_ICONS, CLASS_FOCUSSED, CLASS_SELECTED, REF_INVALID_REGEX } from './constants';
import { addListenerToCollectionElems, alterClass, alterClassOfCollection, escapeHtml, findCommitElemWithId, formatCommaSeparatedList, getCommitElems, handledEvent, EventOverlay } from './utils';
import { getWindowGlobals, TargetType, type CommitTarget, type RefTarget, type RepoTarget } from '../types';
import { t } from './i18n';
...
export interface DialogTextareaInput {
...
export class Dialog {
  ...
  /** 依赖注入：EventOverlay 实例 */
  private eventOverlay: EventOverlay;
  /** 依赖注入：关闭对话框和上下文菜单的回调 */
  private closeDialogAndContextMenuFn: (() => void);

  private static readonly WHITESPACE_REGEXP = /\s/gu;

  /**
   * @param eventOverlay EventOverlay 实例
   * @param closeDialogAndContextMenuFn 关闭对话框和上下文菜单的回调
   */
  constructor(eventOverlay: EventOverlay, closeDialogAndContextMenuFn: (() => void)) {
    this.eventOverlay = eventOverlay;
    this.closeDialogAndContextMenuFn = closeDialogAndContextMenuFn;
  }
  	...
  	const inputRowsHtml = inputs.map((input, id) => {
          ...
          input.info ?
            ...
            '</span></td>';
  				...
  				inputHtml =
            '<td class="inputCol"><textarea id="dialogInput' +
            id +
            '"' +
            ...
            '" rows="' +
            (input.lines || 4) +
            '" style="resize:vertical;width:100%;font-family:inherit;padding:4px;box-sizing:border-box;margin-top:2px;">' +
  		...
  		return (
  			'<tr' +
        (input.type === DialogInputType.Radio ? ' class="mediumField"'
        	: input.type !== DialogInputType.Checkbox ? ' class="largeField"'
        		: '') +
        '>' +
...
this.closeDialogAndContextMenuFn();
  ...
  private render(open: boolean) {
  	...
  	if (this.open !== open) {
          ...
          'customSelectOptions' + (this.data.multiple ? ' multiple' : '');
  			const icon =
          this.data.multiple ? '<div class="selectedIcon">' + SVG_ICONS.check + '</div>' : '';
